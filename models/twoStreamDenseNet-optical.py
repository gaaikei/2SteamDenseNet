   ...
   # (Updated)
    ...
    def composite_function(self, _input, out_features, kernel_size=3):
        ...
        with tf.variable_scope("composite_function"):
            ...
            # convolution
            output = self.conv3d(
                output, out_features=out_features, kernel_size=kernel_size,kernel_type='spatial')
    ...
    # (Updated)
    def add_block(self, _input, growth_rate,kernel_type, layers_per_block):
        """Add N H_l internal layers"""
        output = _input
        for layer in range(layers_per_block):
            with tf.variable_scope("layer_%d" % layer):
                output = self.add_internal_layer(output, growth_rate,kernel_type)
        return output
    ...
    # (Updated)
    def conv3d(self, _input, out_features, kernel_size, kernel_type,
    strides=[1, 1, 1, 1, 1], padding='SAME'):
        ...
        if kernel_type = 'spatial':
            ...
            kernel = self.weight_variable_msra(
                [kernel_size, kernel_size, 2, in_features, out_features],
                name='kernel_temporal')
        else:
            print('please specify your kernel type(spatial or temproal)')
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope("NetworkFusion2"):
            #spatial_temporal_output = tf.multiply(spatial_output, temporal_output)
            spatial_temporal_output_s = spatial_output + temporal_output*0.5 
            spatial_temporal_output_t = spatial_output*0.5 + temporal_output 
        ...
        with tf.variable_scope("temporal_Block_3"):
            temporal_output = self.add_block(spatial_temporal_output, growth_rate, layers_per_block)
