   ...
   # (Updated)
    ...
    should_print=True):
        ...
        summary = tf.Summary(value=[
            ...
            # tf.Summary.Value(tag='predictions_s_%s'%prefix, histo=self.pres_s_hist)
        ])
    ...
    def relu(self,_input):
        # output = tf.nn.leaky_relu(_input)
    ...
    def trainsition_layer_to_classes(self, _input):
        ...
        with tf.name_scope("pooling"):
            output = self.pool2d(output, k = last_pool_kernel_height,
    ...
    #2d conv with batch_size=1 and tensor reshape
    def conv2d(self, _input, out_features, kernel_size,
    strides=[1, 1, 1, 1], padding='SAME'):
        in_features = int(_input.get_shape()[-1])
        input_shape = _input.shape
        if len(_input.shape)==5:
            _input = _input.reshape((input_shape[1],input_shape[2],input_shape[3],input_shape[4]))
        kernel = self.weight_variable_msra(
            [kernel_size,  kernel_size, in_features, out_features],
            name='kernel')
        with tf.name_scope("2DConv"):
    ...
    #2d pooling
    def pool2d(self, _input, k, width_k=None, type='avg', k_stride=None, k_stride_width=None):
        if not width_k: width_k = k
        ksize = [1,  k, width_k, 1]
        if not k_stride: k_stride = k
        if not k_stride_width: k_stride_width = k_stride
        strides = [1, k_stride, k_stride_width, 1]
        padding = 'SAME'
        if type is 'max':
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope("spatial_Initial_convolution"):
            ...
            spatial_output = self.conv2d(
                ...
                strides=[1, 2, 2, 1])
            # [?,16,16,16,24]
            spatial_output = self.pool2d(spatial_output, k=3,type='max', k_stride=2)
            

        with tf.variable_scope("temporal_Initial_convolution"):
            # self.dynamic shape : [None, 16, 64, 64, 3]
            temporal_output = self.conv2d(
            self.dynamic,
            out_features=self.first_output_features,
            kernel_size=3,
            strides=[1, 2, 2, 1])
            # [?,16,16,16,24]
            temporal_output = self.pool2d(temporal_output, k=3,type='max', k_stride=2)
        ...
        # #fusion1
        # with tf.variable_scope("NetworkFusion1"):
        ...
        #fusion2
        with tf.variable_scope("NetworkFusion2"):
            #spatial_temporal_output = tf.multiply(spatial_output, temporal_output)
            spatial_temporal_output = spatial_output + temporal_output 
            #spatial_temporal_output_t = spatial_output + temporal_output 
        ...
        tf.summary.histogram('spatial_prediction', spatial_prediction)
        tf.summary.histogram('temproal_prediction', temproal_prediction)
    ...
    def train_all_epochs(self, train_params):
        ...
        for epoch in range(start_epoch, n_epochs + 1):
            ...
            print("Time per epoch: %s" % str(timedelta(seconds=time_per_epoch)))

            print('Saving model of epoch %d' % epoch)
            ...
            time_validation_ = time.time() - start_valid_time
            time_all_epoch = time.time() - start_time
            seconds_left = int((n_epochs - epoch) * time_all_epoch)
            print("Time for valid: %s" % str(timedelta(seconds=time_validation_)))
            print("Time per epoch: %s, Est. complete in: %s" % (
                str(timedelta(seconds=time_all_epoch)),
