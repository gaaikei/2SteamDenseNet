...
class Data(object):
    ...
    def get_frames_data(self, filename, start_index, sequence_length=16):
        ...
        for parent, dirnames, files in os.walk(filename):
            ...
            #print('filenames_int',filenames_int)
    ...
    def extract_video_data(self):
        ...
        while True:
            ...
            dynamic_path =os.path.join(self.path, 'hmdb51_dynamic/', video_path)
            files = os.listdir(dynamic_path)
            clip_num = len(files)-self.sequence_length
            if clip_num <=0:
                # print(dynamic_path)
                # print ('files length - self.sequence_length:' ,clip_num)
                continue
            s_index = random.randint(0, len(files) - self.sequence_length)
            dynamic = self.get_frames_data(dynamic_path, s_index, self.sequence_length)
            frames = self.get_frames_data(frame_path, s_index, self.sequence_length)
            if dynamic is not None and len(dynamic) == self.sequence_length and frames is not None and len(frames) == self.sequence_length :
                ...
                # print(qsize)
                # print(cnt%7,time.clock() - t0,'seconds per video clip')
                # cnt=(cnt+1)%7
...
class DataProvider(object):
    def __init__(self, path, num_classes, validation_set=None, test=False,
        ...
        train_videos_labels = self.get_path_and_label(os.path.join(self._path, 'train_new.list'))
